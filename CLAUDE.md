...
**The `firebase` project's expected baseline is 15 of 17 passing, not green.** The two upload specs in its *Cloud Storage* block (`uploading an avatar…`, `uploading a private file…`) cannot pass while the connected project has `NEXT_PUBLIC_FIREBASE_STORAGE_ENABLED=false`; the block's other two specs pass because they reject before touching the bucket. Those two failures are the environment, not a regression — don't "fix" them, and don't read them as licence to ignore any *other* red spec.
...
That trust boundary dictates an **ordering invariant in `components/auth/AuthProvider.tsx`**, and it is easy to "simplify" away: `hydrate()` mints the cookie and **awaits it before** `setUser`. Publishing `user` first opens a window where the header shows a signed-in user while every guarded route still sees a guest — which is exactly what used to bounce fresh signups from `/profile` and `/admin` back to `/login`. Minting first exposes three races, all guarded by refs in that file: `authOp` (bumped per transition, so a `hydrate` that loses to a logout never publishes its stale user), `signingOut` (short-circuits the `onAuthStateChanged` bridge fallback so it can't restore a session that is milliseconds from being cleared), and `pendingSync` (the in-flight mint, which `logout()` awaits before clearing so a late mint can't silently sign the user back in). Commit `32f21c8` is the full account.
...
### CMS-driven content
`lib/cms/store.ts` serves page/section content and the program catalog from `.data/cms.json`, **seeded on first read from the static defaults in `lib/site.ts`** so public pages always render before anything is edited. The admin panel (`app/admin/**`, 34 modules) edits these docs/collections. When adding editable content, follow this seed-with-fallback pattern rather than hardcoding.
